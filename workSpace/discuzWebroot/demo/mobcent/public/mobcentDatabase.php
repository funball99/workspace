...
class mobcentGetInfo
{
	...
	{
		$query = DB::fetch_first('SELECT user_access_token, user_access_secret FROM '.DB::table('home_access').' WHERE user_id =%d',array($uid));
		return $query;
	}
	public function sel_accessTopkent($accessSecret,$accessToken)
	{
		$query = DB::fetch_first('SELECT * FROM '.DB::table('home_access').' WHERE user_access_token =%s AND user_access_secret = %s',array($accessToken,$accessSecret));
		return $query;
	}
	
	public function inser_accessTopkent($accessToken,$accessSecret,$uid,$time)
	{
		$query = DB::query('INSERT INTO '.DB::table('home_access').' VALUES(user_access_id,%s,%s,%d,%s)',array($accessToken,$accessSecret,$uid,$time));
		return $query;
	}
	public function update_accessTopkent($accessToken,$accessSecret,$uid,$time)
	{
		$query = DB::query('UPDATE '.DB::table('home_access').' SET user_access_secret = %s,create_time = %s WHERE user_id =%d AND user_access_token =%s',array($accessSecret,$time,$uid,$accessToken));
		return $query;
	}
