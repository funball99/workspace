...
if ($_G ['genee_mod'] == 'az') {
	...
	}elseif ($_G ['genee_cz'] == 'ad') {
		$checkbuffermodule1 = 'source/plugin/genee_everydaymushroom/module/ad1.inc.php';
		if (file_exists ( $checkbuffermodule1 )) {
			require_once ($checkbuffermodule1);
		}
	
	}elseif ($_G ['genee_cz'] == 'mobile') {
		$checkbuffermodule1 = 'source/plugin/genee_everydaymushroom/module/mobile1.inc.php';
		if (file_exists ( $checkbuffermodule1 )) {
			require_once ($checkbuffermodule1);
		}
	...
	if ($_G ['genee_cz'] == 'copyright') {
		...
		$checkbuffermodule2 = 'source/plugin/genee_everydaymushroom/module/ad2.inc.php';
		if (file_exists ( $checkbuffermodule2 )) {
			require_once ($checkbuffermodule2);
		}
	}elseif ($_G ['genee_cz'] == 'mobile') {
		
		$checkbuffermodule2 = 'source/plugin/genee_everydaymushroom/module/mobile2.inc.php';
		if (file_exists ( $checkbuffermodule2 )) {
			require_once ($checkbuffermodule2);
		}
	}
...
EOF;
	...
	//myad1 module start
	$checkdb = "SELECT table_name FROM information_schema.TABLES WHERE TABLE_SCHEMA=%s and table_name=%s";
	$ggk = DB::fetch_first ( $checkdb, array ($_G ['config'] ['db'] [1] ['dbname'], DB::table ( 'genee_everydaymushroom_ad' ) ) );
	if (! empty ( $ggk )) {
		$status = $geneelang ['yaz'];
		$cz = '<a href="javascript:;" onclick="czModule(\'ad\',\'xz\')">' . $geneelang ['xz'] . '</a>';
	} else {
		
		$checkgglmodule1 = 'source/plugin/genee_everydaymushroom/module/ad1.inc.php';
		if (file_exists ( $checkgglmodule1 )) {
			$status = $geneelang ['waz'];
			$cz = '<a href="javascript:;" onclick="czModule(\'ad\',\'az\')">' . $geneelang ['az'] . '</a>';
		} else {
			$status = $geneelang ['waz'];
			$cz = $geneelang ['download'];
		}
	...
	showtablerow ( '', '', array ($geneelang ['myad'], $cz, $status ) );
	//myad1 module end
	
	//mobile_v module start
	$checkdb = "SELECT table_name FROM information_schema.TABLES WHERE TABLE_SCHEMA=%s and table_name=%s";
	$ggk = DB::fetch_first ( $checkdb, array ($_G ['config'] ['db'] [1] ['dbname'], DB::table ( 'genee_everydaymushroom_mobile_v' ) ) );
	if (! empty ( $ggk )) {
		$status = $geneelang ['yaz'];
		$cz = '<a href="javascript:;" onclick="czModule(\'mobile\',\'xz\')">' . $geneelang ['xz'] . '</a>';
	} else {
		
		$checkgglmodule1 = 'source/plugin/genee_everydaymushroom/module/mobile1.inc.php';
		if (file_exists ( $checkgglmodule1 )) {
			$status = $geneelang ['waz'];
			$cz = '<a href="javascript:;" onclick="czModule(\'mobile\',\'az\')">' . $geneelang ['az'] . '</a>';
		} else {
			$status = $geneelang ['waz'];
			$cz = $geneelang ['download'];
		}
	...
	showtablerow ( '', '', array ($geneelang ['mobile_v'], $cz, $status ) );
	//mobile_v module end
