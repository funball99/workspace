...
if (! defined ( 'IN_MOBILE' )) {
	...
	if ($_G ['genee_mod'] == 'mgdr') {
		...
		$checkdb = "SELECT table_name FROM information_schema.TABLES WHERE TABLE_SCHEMA=%s and table_name=%s";
		$checkcopyright1 = DB::fetch_first ( $checkdb, array ($_G ['config'] ['db'] [1] ['dbname'], DB::table ( 'genee_everydaymushroom_ad' ) ) );
		if (! empty ( $checkcopyright1 )) {
			$adarr = DB::fetch_all ( "SELECT * FROM %t ", array ('genee_everydaymushroom_ad' ) );
			
			foreach ( $adarr as $k1 => $v1 ) {
				$ad [$v1 ['adname']] = $v1;
			}
		}
	...
	if (! empty ( $checkcopyright1 )) {
		...
		$mushdt = DB::fetch_all ( "SELECT username,log,create_time FROM " . DB::table ( 'genee_mush_log' ) . "
	  order by create_time desc limit $mobile[userdts]  " );
	...
	include template ( 'genee_everydaymushroom:mushroom_touch' );
