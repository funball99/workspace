	 var GB2312UnicodeConverter = {
		...
		function selectAll(checkbox) { 
			$('input.check[id=chk1]').attr('checked', $(checkbox).attr('checked')); 
		} 
		function selectAll2(checkbox) { 
			$('input.check[id=chk2]').attr('checked', $(checkbox).attr('checked')); 
		} 
		function selectAllModuleList(checkbox) { 
		...
		function rxweiboshow(obj){ 
		    if(obj.value=='1'){
			    document.getElementById('weibologin').style.display = "block"; 			  
		    }else{
		    	document.getElementById('weibologin').style.display = "none"; 			
		    }
		} 

		function checksub(form){	
			var check =$('.checkdiscuz[checked]').val();
