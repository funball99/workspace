...
class FavoriteInfoImpl_x20 extends abstractFavoriteInfo {
	public function getAddFavoriteInfoObj() {
		...
		public function getDelFavoriteInfoObj()
		{
			...
			if(!$_G['uid']){
				echo common_json::on_error_content('\u8bf7\u767b\u5f55\u540e\u64cd\u4f5c');exit();
			}
			$_GET['type']		=$_GET['type'] ?$_GET['type']:'thread';
			...
			$_GET['spaceuid'] 	= $_GET['spaceuid']?$_GET['spaceuid']:0;
			$_POST['favoritesubmit'] = $_POST['favoritesubmit']?$_POST['favoritesubmit']:true;
			$_POST['description'] = $_POST['description']?$_POST['description']:Common::get_unicode_charset('\u901a\u8fc7\u5176\u4ed6\u65b9\u5f0f\u6536\u85cf');
			...
			if($_GET['checkall']) {
				...
				if(empty($thevalue) || $thevalue['uid'] != $_G['uid']) {
					echo common_json::on_error_content('\u62b1\u6b49\uff0c\u60a8\u6307\u5b9a\u7684\u6536\u85cf\u4e0d\u5b58\u5728');exit();
				}else {
