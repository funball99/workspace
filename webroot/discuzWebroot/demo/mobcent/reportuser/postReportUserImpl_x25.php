...
class postReportUserImpl_x25 extends abstractReportUser {
	public function getReportUserObj() {
		...
		$userId = $_G ['uid'] =$arrAccess['user_id'];
		if(empty($_G ['uid']))
		{
		...
		$message = $space['username'].'&nbsp;:&nbsp;'.rtrim($message, "\\");
		$reportArray = DB::fetch_first("SELECT id FROM ".DB::TABLE('common_report')."  WHERE urlkey = '".$urlkey."' AND uid = ".$userId);
