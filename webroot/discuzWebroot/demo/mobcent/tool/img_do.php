...
class ThumbHandler {
	...
	function createImg($a, $b = null) {
		...
		if (1 == $num) {
			$r = ( int ) $a;
			if ($r < 1) {
				die ( "Image scaling should not be less than 1" );
			}
			$this->img_scale = $r;
		...
		if (2 == $num) {
			...
			$h = ( int ) $b;
			if (0 == $w) {
				die ( "The target width is not zero" );
			}
			if (0 == $h) {
				die ( "Target height cannot be zero" );
			}
	...
	function _createMask() {
		if ($this->mask_word) {
			...
			if ($this->_isFull ()) {
				die ( "Watermark text is too large" );
			} else {
	...
	function _setNewImgSize($img_w, $img_h = null) {
		...
		if (2 == $num) {
			...
			if ($fill_w < 0 || $fill_h < 0) {
				die ( "Photo frame is too big, more than the width of the image" );
			}
	...
	function _checkMaskValid() {
		if ($this->mask_w + $this->mask_offset_x > $this->src_w || $this->mask_h + $this->mask_offset_y > $this->src_h) {
			die ( "Watermark image size is larger than the original, please narrow the watermark" );
		}
	}
	...
	function _getImgType($file_path) {
		...
		if (file_exists ( $file_path )) {
			...
			die ( "File does not exist, cannot achieve the file type!" );
		}
	}
...
function createImg($img_data, $img_name, $size) {
	
	if (is_file ( $img_data )) {
		$t = new ThumbHandler ();
		$t->setSrcImg ( $img_data );
		$t->setCutType ( 0 ); 
		$t->setDstImg ( $img_name );
		return $t->createImg ( $size, $size );
	} else {
		return false;
	}
}
